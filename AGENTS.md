...
每个 Phase 完成后，**彻底清理** `currentTasks.md` 和 `todo.md`：

- `currentTasks.md`：删除已完成 Phase 的详细步骤（背景、文件列表、每步勾选），仅保留一行摘要；通过验收的条目标 `[x]`
- `todo.md`：删除已完成条目的全部内容（标题、描述、目录结构），不留已完成记录
- 清理后 `todo.md` 只保留未开始的待办项；全完成则为「（暂无待办项）」
