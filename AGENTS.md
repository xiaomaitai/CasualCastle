...
项目采用**六边形架构（Hexagonal Architecture）**与 **DDD（领域驱动设计）** 规范开发，代码按三层放在 `scripts/`：

> 详细项目结构与模块依赖见 `devPlan/codeStructure.md` 和 `devPlan/outline/c03ProjectArchitecture.md`。

| 目录 | 层级 | 用途 |
| --- | --- | --- |
| `domain/` | 核心域 | 纯 C# 业务规则（coordinates、card、combine、core、night、battle），零 Godot |
...
| 需要了解     | 读取                                          |
| -------- | ------------------------------------------- |
| 架构 backlog   | `devPlan/tmp/todo.md`                           |
| 当前任务、验收项 | `devPlan/tmp/currentTasks.md`                   |
| 里程碑与路线图  | `devPlan/outline/`                          |
| 模块设计与依赖  | `devPlan/outline/c05ProjectArchitecture.md` |
| 组合 / AI 规则 | `devPlan/design/combineSystem.md`、`devPlan/design/battleReport.md`、`devPlan/design/replayAi.md` |
| UI 设计 | `devPlan/design/uiDesign.md` |
| 素材规格 | `devPlan/design/assetSpecs.md` |
| 依赖注入 | `devPlan/design/dependencyInjection.md` |
...
`devPlan/tmp/` 下有两个临时文档，用于追踪当前施工进度：

| 文件 | 用途 | 何时写入 | 何时清理 |
|------|------|---------|---------|
| `currentTasks.md` | 当前正在做的任务的细化步骤和验收项 | 用户要求细化任务时 | 任务全部完成时，**直接删除文件全部内容，不需要保留总结** |
| `todo.md` | 长期架构 backlog，当前不想做、以后可能做的想法 | 发现需要记录但不立即处理的问题时 | 某项确定要做时移出；确定不做时删除该项 |

**清理规则**：
- 清理时直接删除全部内容，让文件变空，不写"已完成"、"历史记录"等总结
- 当用户说"继续当前任务"时，先查 `currentTasks.md`；如果已完成就查大纲找下一个 Phase
- 当用户要求把新任务细化到当前任务时，先彻底清理 `currentTasks.md` 和 `todo.md` 中已完成的内容

## 工作规则

- 当用户要求继续当前任务时，在查找 `devPlan/tmp/currentTasks.md`，如果里面所有任务已经完成，就在大纲中查找下一个 Phase 的任务，询问用户是否细化到当前任务。**不要**直接在 `devPlan/tmp/todo.md` 中查找
- 实现任务时若发现**需要重构**的结构（职责混乱、重复逻辑、模块边界不清、技术债阻碍当前改动），在回复中**明确提出来**，说明问题、影响和建议方向。
- 不要擅自扩大范围做大规模重构；是否纳入当前任务或记入 `devPlan` 由用户决定
- 一旦进行大规模重构后，要更新文档同步改动
...
自定义钩子放在 `githooks/`，clone 后需执行一次：

    git config core.hooksPath githooks

`assets/data/config_db_dump.sql` 由 pre-commit hook 自动 dump 生成，**不要手动阅读或编辑此文件**。修改数据库应直接操作 `config.db`，提交时 hook 会自动更新 dump。
...
- `assets/data/config.db` 未入库（`.gitignore`），需用 `sqlite3` 从 SQL 文本重建；缺失该 db 游戏无法正确加载数据。云 agent 的 `core.hooksPath` 指向别处，`githooks/` 的 checkout/merge 重建不生效，故改由启动更新脚本重建。
- **数据源当前以 `devPlan/design/database/seed.sql` 为准，而非 `assets/data/config_db_dump.sql`。** 提交的 `config_db_dump.sql` 已过时（缺 `shop_catalog.weight` 列、用旧表名 `fusion_recipes`、缺 P3 人类种族单位/建筑数据），用它重建会导致 `GameDataLoader` 抛 `SQLite Error 1: 'no such column: weight'` 且标题界面/DI 崩溃。`seed.sql` 与运行时代码（`GameDataLoader`/`SqliteUnitRepository`/`SqliteBuildingRepository`）的 schema 完全一致。启动更新脚本用 `seed.sql` 重建。若哪天 `config_db_dump.sql` 被同步为最新，可再切回 dump。
