...
- `assets/data/config.db` 未入库（`.gitignore`），由 `assets/data/config_db_dump.sql` 用 `sqlite3` 重建。仓库 `githooks/` 会在 checkout/merge 时重建，但云 agent 的 `core.hooksPath` 指向别处，因此改由启动更新脚本重建；缺失该 db 游戏无法正确加载数据。
