...
战报是 M6 的**核心产出**：记录玩家每一夜结束时的城堡快照，永久保存后供敌方「回放式 AI」复刻。概念定义见 `concepts.md`；敌方复刻规则见 `aiSystemDesign.md`。
...
| `ReplayAiSystem` | 开局选战报、入夜复刻（见 `aiSystemDesign.md`） |
