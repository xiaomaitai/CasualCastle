...
public class TechTreeEditorController
{
    ...
    private readonly TabBar _raceTabBar;
    ...
    private List<RaceDef> _races;
    private List<TechTreeNode> _nodes;
    private List<CombineRecipe> _edges;
    ...
    {
        _root = root;
        _raceTabBar = root.GetNode<TabBar>("TopBar/RaceTabBar");
        ...
        _raceTabBar.TabClicked += OnRaceTabClicked;
    ...
    public void Process()
    {
        ...
        if (_dragMode == DragMode.MoveCard)
        {
            if (_dragPreview != null && GodotObject.IsInstanceValid(_dragPreview))
            {
                ...
                int tier = (int)_dragSource.GetMeta("tier");
                float rowY = (4 - (tier - 1)) * rowHeight + (rowHeight - CardHeight) / 2;
                Vector2 localPos = _cardCanvas.GetLocalMousePosition();
                _dragPreview.Position = new Vector2(localPos.X - _dragOffset.X, rowY);
            }
        }
    ...
    private void LoadRaces()
    {
        ...
        _raceTabBar.ClearTabs();
        foreach (RaceDef race in _races)
            _raceTabBar.AddTab(race.DisplayName);

        if (_races.Count > 0)
        {
            _raceTabBar.CurrentTab = 0;
            LoadRace(_races[0].Id);
        }
    }

    private void LoadRace(string raceId)
    {
        _currentRaceId = raceId;
        _nodes = _repo.LoadNodes(raceId);
        _edges = _repo.LoadEdges(raceId);
        LayoutCards();
        RefreshLibrary();
    ...
    private Panel CreateCardView(TechTreeNode node)
    {
        ...
        styleBox.BgColor = node.ShopAvailable
        ...
        Label nameLabel = new()
        {
            ...
            Size = new Vector2(CardWidth - 8, 20),
            HorizontalAlignment = HorizontalAlignment.Center
        };
        panel.AddChild(nameLabel);

        if (node.ShopAvailable)
        {
            Label costLabel = new()
            {
                Text = $"{node.GoldCost}g",
                Position = new Vector2(4, 22),
                Size = new Vector2(CardWidth - 8, 16),
                HorizontalAlignment = HorizontalAlignment.Center
            };
        ...
        {
            Text = string.IsNullOrEmpty(node.UnitTypeId) ? "" : $"→{node.UnitTypeId}",
    ...
    private void EndConnectionDrag()
    {
        ...
        {
            string fromId = _dragSource.GetMeta("type_id").AsString();
            string toId = targetCard.GetMeta("type_id").AsString();
    ...
    private void CanvasToLibrary(Panel card)
    {
        ...
        foreach (CombineRecipe recipe in affectedRecipes)
        {
            ...
            {
                _repo.RemoveRecipe(recipe.MainTypeId, recipe.MaterialTypeId);
                continue;
            }
    ...
    private void TryDeleteEdgeAt(Vector2 canvasPos)
    {
        ...
        if (toDelete != null)
        {
            _repo.RemoveRecipe(toDelete.MainTypeId, toDelete.MaterialTypeId);
            _edges = _repo.LoadEdges(_currentRaceId);
            _cardCanvas.QueueRedraw();
    ...
    private void OnRaceTabClicked(long tab)
    {
        if (tab < _races.Count)
            LoadRace(_races[(int)tab].Id);
    }
